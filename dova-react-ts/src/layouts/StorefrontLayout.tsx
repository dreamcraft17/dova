import { Link } from 'react-router-dom';
import type { ReactNode } from 'react';
import { PortalUIProvider, Toast } from '../components/PortalUI';

export default function StorefrontLayout({ children }: { children: ReactNode }) {
  return (
    <PortalUIProvider>
    <div className="storefront-app">
      <div className="top">
        <div className="container">
          <span>DOVA CHAIN · FOOD SUPPLY CHAIN</span>
          <span>Nigeria · Starting with Plantain Flour</span>
        </div>
      </div>
      <header className="nav">
        <div className="container navin">
          <Link className="logo" to="/">DOVA<i>CHAIN</i></Link>
          <nav className="links" aria-label="Storefront navigation">
            <Link to="/">Home</Link>
            <Link to="/marketplace">Products</Link>
            <Link to="/bundles">Bundles</Link>
            <Link to="/about">About</Link>
            <Link to="/contact">Contact</Link>
            <Link to="/feedback">Feedback</Link>
          </nav>
          <div className="actions">
            <Link className="icon" to="/marketplace" aria-label="Search">⌕</Link>
            <Link className="icon" to="/cart" aria-label="Cart">🛒</Link>
            <Link className="btn gold" to="/marketplace">Shop</Link>
          </div>
        </div>
      </header>
      <main>{children}</main>
      <StorefrontFooter />
      <Toast />
    </div>
    </PortalUIProvider>
  );
}

function StorefrontFooter() {
  return (
    <footer className="footer">
      <div className="container">
        <div className="footgrid">
          <div>
            <strong style={{letterSpacing:'0.12em'}}>DOVA<span style={{color:'var(--gold)'}}>CHAIN</span></strong>
            <p>From Farm to Table. On Time, Every Time.</p>
            <p>Building a technology-enabled food supply chain, starting with Plantain Flour.</p>
          </div>
          <div>
            <h4>SHOP</h4>
            <Link to="/marketplace">Products</Link>
            <Link to="/bundles">Bundles</Link>
            <Link to="/cart">Cart</Link>
          </div>
          <div>
            <h4>COMPANY</h4>
            <Link to="/about">About Us</Link>
            <Link to="/contact">Contact Us</Link>
            <Link to="/feedback">Feedback</Link>
          </div>
          <div>
            <h4>NETWORK</h4>
            <Link to="/#farmers">Farmers</Link>
            <Link to="/#businesses">Businesses</Link>
            <a href="https://dova.dntech.id/chat">DOVA AI</a>
          </div>
        </div>
        <div className="bottom">
          <span>© 2026 DOVA Chain. All rights reserved.</span>
          <span>officialdovachain@gmail.com · Nigeria</span>
        </div>
      </div>
    </footer>
  );
}
