...
We’re still inside the first week of the 4-week MVP window (21 Jul – 17 Aug), but a lot of the shopping and supplier/admin work that was planned for weeks 2–3 is already working in the product. You can walk through the main flows on an internal demo. We’re **not** live for the public yet, and we’re **not** taking real money yet.

Quick picture:

- Internal demo: yes  
- Public live site: no  
- Real payment charges: no (demo uses a simulated payment unless Paystack test keys are connected)  
...
### Customers
Sign-up, login, and logout are in. Product browsing works (search, categories, product pages). Cart works. Checkout creates an order. There’s a confirmation step and order history in the customer area. Payment is connected end-to-end in the product; on the current demo it runs as a **simulated** payment so we don’t charge real money until Paystack is set up properly.

### Suppliers
Registration with document upload is in. New suppliers start as pending. Once an admin approves them, they get a dashboard: add/edit/remove products, adjust stock, and move orders through fulfillment (e.g. processing → shipped → delivered).

### Admins
Admin login and dashboard are in — platform overview, supplier approve/reject, and visibility into users, products, and orders.

### Public pages
Home, About Us, Contact, plus the shared header/footer, are in place.
...
**Week 4** (polish, fuller testing, production launch): not started.

So: the product side of the MVP is further along than the calendar suggests. The “is this ready to show customers / take payments for real” side is still open.
...
Things we deliberately left out of this MVP (so nobody expects them yet): password reset, email verification, reviews, wishlist, discounts, courier tracking.

---

## Hosting (as currently planned)

The customer site is aimed at Vercel. The backend runs separately. Database and session storage are planned as managed services. Payments for MVP stay on Paystack only. We’re not using Docker in the day-to-day workflow for this project.

---

## Checks we’ve already run

Automated unit tests are passing (24). Auth smoke checks pass. The project builds cleanly. We have **not** finished full browser testing on a public staging site, and we have **not** done production performance checks.
